...
2. Provide pushback information for all users (including ones who do not use GSX).   
...
2. Move the `.ini`, `.py`, and `_handler.py` files to `%AppData%\Virtuali\GSX\MSFS`. There is no need to change AFCAD `.bgl` file path for MSFS GSX Pro version.
3. Move `.py` files from `lib` folder in the repository to `%APPDATA%\Virtuali\Handlers\lib`. More on https://gsjp.info/gsx-handlers/.
